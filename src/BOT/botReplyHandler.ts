  ...
  switch (text) {
    ...
    case infoMess.chatNamesFilterOpt:
      ...
      if (!chatsByChatFilter?.chats?.length) {
        ...
        await bot.sendMessage(
          chat_id,
          settings[chat_id].searchType === buttonTexts.GPTSearch ? infoMess.writeTopic : infoMess.writeKeyWordsForTopic,
          options.InputValue,
        );
      }
      break;
    ...
    case infoMess.chatNames:
      if (text === infoMess.chatNames) {
        ...
        if (!chatsByChatNames?.chats?.length) {
          await bot.sendMessage(chat_id, 'Anyone chat not find, please change filter', menuOptions.SearchSettings);
        } else {
