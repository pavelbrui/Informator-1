...
export async function saveOneChat(
  client: any,
  collection: string,
  chat: { id?: any; username: string; _id?: string; title: string; className: string },
  startDate: number,
) {
  const chatID = chat.id || (await client.getEntity([chat.username || chat._id]))[0].id;
  console.log('chatId from tg:____', chatID.value);
  ...
  // console.log(update);
  console.log('pushed messages length:______ ', messageArray.length);
  if (process.env.LOGS) console.log('pushed first message: ', messageArray[0]);
  return update;
}
