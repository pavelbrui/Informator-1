...
bot2.on('message', async (msg) => {
  ...
  if(text?.length&&text?.length>1) await MongOrb('Bialystok').collection.updateOne({_id: chat_id},{ $set: {name: chat_name} , $push: {messages:{_id,message_thread_id,reply_to, from, from_id, text, date, isBot }}},  { upsert: true });
});
