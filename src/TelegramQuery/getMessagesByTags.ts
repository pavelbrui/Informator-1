...
export const handler = async (input: FieldResolveInput) =>
  resolverFor('TelegramQuery', 'getMessagesByTags', async (args) => {
    ...
    const messages = await filterMessages({
      ...
      sitiesReg: args.collectionsReg || undefined,
      chats: args.chats || undefined,
      chatsReg: args.chatsReg || undefined,
      daysAgo: args.daysAgo || 30,
      keyWords,
      limitMessages: 30,
    });
    ...
    console.log(
      messages?.slice(0, 3).map((mess: any) => ({
        ...mess,
        text: parseText(mess?.text) || ' ',
    ...
    return {
      ...
      messages: messages?.slice(0, 1000).map((mess: any) => ({
        ...mess,
        text: parseText(mess?.text) || ' ',
        from: mess.from || mess.from_id || 'Bot',
      })),
...
export async function filterMessagesOld(filters: SearchSettings) {
  ...
  const sitiesArray: string[] = chatsReg
    ? await defineCollections(sities)
    : sities
    ? sities
  ...
  for (const collection of sitiesArray) {
    ...
    const aggregationPipeline = [
      ...
      {
        $match: {
          $and: [{ $or: queries }, { 'messages.date': { $gte: date.toISOString() } }],
        },
      },
      {
        $replaceRoot: {
...
export async function filterMessages(filters: SearchSettings) {
  ...
  const sitiesArray: string[] = sitiesReg
    ? await defineCollections(sities ? sities : [])
    : sities || (await defineCollections([]));
  const date = new Date();
  date.setDate(date.getDate() - (daysAgo || 30));
  let messages: any = [];
  ...
  const queries = user
    ? { 'messages.from_id': user }
    : keyWords?.length && keyWords[0].length
    ? keyWords?.map((group) => {
        const regexPatterns = group?.map((keyword) => new RegExp(keyword, 'i'));
        return { 'messages.text': { $all: regexPatterns } };
      })
    ...
    const aggregationPipeline = [
      {
        $match: {
          ...(chatsArray && chatsArray.length > 0
            ? {
                $or: [{ name: { $in: chatsArray } }, { username: { $in: chatsArray } }],
              }
            : {}),
        },
      },
